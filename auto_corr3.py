...
def main():
   ...
   parser.add_option("-f",metavar="FILE",help="Trajectory",default="traj.trr")
   ...
   temp_x=[]
   temp_f=[]
   trn_v=N.empty((nP,tf.natoms/3,3,3),dtype=N_real)
   trn_x=N.empty((nP,tf.natoms/3,3,3),dtype=N_real)
   ...
   for i,frm in enumerate(tf):
       ...
       if __debug__:
           ...
           N.testing.assert_almost_equal(a,b,decimal=3)  #low accuracy dominated by pressure coupling
   ...
   trn_w=N.empty((len(trn_v),tf.natoms/3,3),dtype=N_real)

   #COM
   trn_x -= N.sum(trn_x*mass,axis=2).reshape(nP,tf.natoms/3,1,3)/N.sum(mass)  #reshape doesn't change shape but reinserts the summed axis
   trn_v_com = trn_v - N.sum(trn_v*mass,axis=2).reshape(nP,tf.natoms/3,1,3)/N.sum(mass)

   for n in range(len(trn_v)):
       trn_w[n] = compute_omega(trn_x[n], trn_v_com[n], ic, I, mass.reshape(3))
       #trn_w[n] = compute_omega_py(trm_x[n], trn_v[n], ic, I, mass)
   ...
   print(trn_v.shape)
   if bCom:
       #trn_v = trn_v.reshape((-1,tf.natoms/3,3,3)) #group as waters
   ...
   if bCom:
       ...
       corr = corr.reshape(-1,tf.natoms/3,3,3) #frames, molecule, atoms, xyz; so that we can multiply with mass
       print(corr.shape)
       corr *= mass
   corr=norm_corr(corr)
   print(corr.shape)
